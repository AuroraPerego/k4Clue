...
std::enable_if_t<not std::is_array_v<T>, host_buffer<T>> make_host_buffer() {
  return alpaka::allocMappedBuf<TPlatform, T, Idx>(
      host(), platform<TPlatform>(), Scalar{});
}
...
// potentially cached, pinned, scalar and 1-dimensional host buffers, associated
// to a work queue the memory is pinned according to the device associated to
// the queue
...
std::enable_if_t<not std::is_array_v<T>, host_buffer<T>> make_host_buffer(
  ...
  if constexpr (allocator_policy<alpaka::Dev<TQueue>> ==
    ...
    return alpaka::allocMappedBuf<Platform, T, Idx>(
        host(), platform<Platform>(), Scalar{});
  }
}
...
make_host_buffer(TQueue const& queue, Extent extent) {
  if constexpr (allocator_policy<alpaka::Dev<TQueue>> ==
                AllocatorPolicy::Caching) {
    return allocCachedBuf<std::remove_extent_t<T>, Idx>(host(), queue,
                                                        Vec1D{extent});
  } else {
    using Platform = alpaka::Platform<alpaka::Dev<TQueue>>;
...
make_device_buffer(TQueue const& queue, Extent extent) {
  if constexpr (allocator_policy<alpaka::Dev<TQueue>> ==
                AllocatorPolicy::Caching) {
    return allocCachedBuf<std::remove_extent_t<T>, Idx>(alpaka::getDev(queue),
                                                        queue, Vec1D{extent});
  }
  if constexpr (allocator_policy<alpaka::Dev<TQueue>> ==
                AllocatorPolicy::Asynchronous) {
...
make_device_buffer(TQueue const& queue) {
  ...
  if constexpr (allocator_policy<alpaka::Dev<TQueue>> ==
    ...
    return alpaka::allocAsyncBuf<std::remove_extent_t<T>, Idx>(
        queue, Vec1D{std::extent_v<T>});
  }
  if constexpr (allocator_policy<alpaka::Dev<TQueue>> ==
                AllocatorPolicy::Synchronous) {
    return alpaka::allocBuf<std::remove_extent_t<T>, Idx>(
        alpaka::getDev(queue), Vec1D{std::extent_v<T>});
  }
}
