...
void ClueGaudiAlgorithmWrapper::fillCLUEPoints(
    std::vector<clue::CLUECalorimeterHit>& clue_hits) const {
...
std::map<int, std::vector<int> > ClueGaudiAlgorithmWrapper::runAlgo(
    std::vector<clue::CLUECalorimeterHit>& clue_hits, bool isBarrel = false) const {
  ...
  if (isBarrel) {
    ...
    if (clueAlgoBarrel_.clearAndSetPoints(
            x.size(), &x[0], &y[0], &layer[0], &weight[0], &r[0]))
    ...
    if (clueAlgoEndcap_.clearAndSetPoints(
            x.size(), &x[0], &y[0], &layer[0], &weight[0], &r[0]))
...
StatusCode ClueGaudiAlgorithmWrapper::execute(const EventContext&) const {
  ...
  debug() << "ClueGaudiAlgorithmWrapper: Number of calo hits: "
