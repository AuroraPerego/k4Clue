...
namespace clue {
  ...
  CLUECalorimeterHit::CLUECalorimeterHit(
  ...
  bool CLUECalorimeterHit::inEndcap() const {
    return (m_detectorRegion == endcap ? true : false);
  }
  bool CLUECalorimeterHit::isFollower() const {
    return (m_status == follower ? true : false);
  }
  ...
  void CLUECalorimeterHit::setR() {
    m_r = float(
        sqrt(getPosition().x * getPosition().x + getPosition().y * getPosition().y));
  }
...
std::ostream& operator<<(std::ostream& o, const clue::CLUECalorimeterHit& value) {
    o << "Energy: " << value.getEnergy() << "\n"
      << "Time: " << value.getTime() << "\n"
      << "Position: (" << value.getPosition().x << ", "
      << value.getPosition().y << ", "
      << value.getPosition().z << ")\n"
      << "Layer: " << value.getLayer() << "\n"
      << "Region: " << (value.inBarrel() ? "Barrel" : "Endcap") << "\n"
      << "Status: " << (value.isSeed() ? "Seed" : value.isFollower() ? "Follower" : "Outlier") << "\n"
      << "Rho: " << value.getRho() << "\n"
