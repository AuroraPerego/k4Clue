...
void CLUEAlgo_T<TILES>::makeClusters() {
  ...
  if (verbose_)
    std::cout << "ClueGaudiAlgorithmWrapper: prepareDataStructures:     "
  ...
  if (verbose_)
    std::cout << "ClueGaudiAlgorithmWrapper: calculateLocalDensity:     "
              << elapsed.count() * 1000 << " ms" << std::endl;
  ...
  elapsed = finish - start;
  if (verbose_)
    std::cout << "ClueGaudiAlgorithmWrapper: calculateDistanceToHigher: "
              << elapsed.count() * 1000 << " ms" << std::endl;
...
void CLUEAlgo_T<TILES>::prepareDataStructures() {
  for (size_t i = 0; i < points_.n; i++) {
    ...
    allLayerTiles_.fill(points_.layer[i],
                        points_.x[i],
                        points_.y[i],
                        points_.x[i] / (1. * points_.r[i]),
                        i);
  }
}
...
void CLUEAlgo_T<TILES>::calculateLocalDensity() {
  ...
  for (size_t i = 0; i < points_.n; i++) {
    ...
    // get search box
    search_box = lt.searchBox(
        points_.x[i] - dc_, points_.x[i] + dc_, points_.y[i] - dc_, points_.y[i] + dc_);

    if (!TILES::constants_type_t::endcap) {
      float dc_phi = dc_ * inv_ri;
      search_box = lt.searchBoxPhiZ(
          phi_i - dc_phi, phi_i + dc_phi, points_.y[i] - dc_, points_.y[i] + dc_);
    }
    ...
    for (int xBin = search_box[0]; xBin <= search_box[1]; ++xBin) {
      for (int yBin = search_box[2]; yBin <= search_box[3]; ++yBin) {
        ...
        for (size_t binIter = 0; binIter < binSize; binIter++) {
          ...
          // query N_{dc_}(i)
          float dist2_ij = TILES::constants_type_t::endcap ? distance2(i, j)
                                                           : distance2(i, j, true, ri);
          if (dist2_ij <= dc2) {
            // sum weights within N_{dc_}(i)
            points_.rho[i] +=
                (i == static_cast<unsigned int>(j) ? 1.f : 0.5f) * points_.weight[j];
          }
  ...
  for (size_t i = 0; i < points_.n; i++) {
    ...
    std::array<int, 4> search_box =
        TILES::constants_type_t::endcap
            ...
            : lt.searchBoxPhiZ(
                  phi_i - dm_phi, phi_i + dm_phi, points_.y[i] - dm, points_.y[i] + dm);
    ...
    for (int xBin = search_box[0]; xBin <= search_box[1]; ++xBin) {
      for (int yBin = search_box[2]; yBin <= search_box[3]; ++yBin) {
        // get the id of this bin
        int phi = (xBin % TILES::constants_type_t::nColumnsPhi);
        int binId = TILES::constants_type_t::endcap ? lt.getGlobalBinByBin(xBin, yBin)
        ...
        for (int binIter = 0; binIter < binSize; binIter++) {
          ...
          foundHigher = foundHigher ||
                        ((points_.rho[j] == rho_i) && (static_cast<unsigned int>(j) > i));
          float dist_ij =
              TILES::constants_type_t::endcap ? distance(i, j) : distance(i, j, true, ri);
...
void CLUEAlgo_T<TILES>::findAndAssignClusters() {
  ...
  std::chrono::duration<double> elapsed = finish - start;
  if (verbose_)
    std::cout << "ClueGaudiAlgorithmWrapper: findSeedAndFollowers:      "
              << elapsed.count() * 1000 << " ms" << std::endl;
  ...
  elapsed = finish - start;
  if (verbose_)
    std::cout << "ClueGaudiAlgorithmWrapper: assignClusters:            "
              << elapsed.count() * 1000 << " ms" << std::endl;
}
