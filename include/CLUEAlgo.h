...
public:
  ...
  CLUEAlgo_T(float dc, float rhoc, float outlierDeltaFactor, bool verbose) {
    ...
    verbose_ = verbose;
    if (verbose_) {
      std::cout << "ClueGaudiAlgorithmWrapper: nTiles (cols,rows):     "
                << TILES::constants_type_t::nTiles;
      std::cout << " ("
                << (TILES::constants_type_t::endcap
                        ? TILES::constants_type_t::nColumns
  ...
  bool clearAndSetPoints(
      int n, float* x, float* y, int* layer, float* weight, float* r = NULL) {
    points_.clear();
    // input variables
    for (int i = 0; i < n; ++i) {
      ...
      if (r != NULL) {
        ...
        if (TILES::constants_type_t::endcap) {
          ...
          std::cerr
              << "ERROR: r info is not present but you are using a barrel LayerTile! "
              << std::endl;
        }
      }
    }
    ...
    if (maxX > TILES::constants_type_t::maxX || minX < TILES::constants_type_t::minX) {
      std::cout << "Min and/or max x element (" << minX << "," << maxX << ")"
                << " are outside the boundaries defined for the current detector ("
                << TILES::constants_type_t::minX << "," << TILES::constants_type_t::maxX
                << ")" << std::endl;
      return 0;
    }
    ...
    if (maxY > TILES::constants_type_t::maxY || minY < TILES::constants_type_t::minY) {
      std::cout << "Min and/or max x element (" << minY << "," << maxY << ")"
                << " are outside the boundaries defined for the current detector ("
                << TILES::constants_type_t::minY << "," << TILES::constants_type_t::maxY
                << ")" << std::endl;
      return 0;
    }
