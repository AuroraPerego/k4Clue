...
public:
  ...
  /**
     ...
     * correct bins.
     */
  std::array<int, 4> searchBoxPhiZ(float phiMin,
                                   float phiMax,
                                   float zMin,
                                   float zMax) const {
...
namespace clue {
  ...
  using CLICdetEndcapTiles =
      std::array<CLICdetEndcapLayerTile, CLICdetEndcapLayerTilesConstants::nLayers>;

  using CLICdetBarrelLayerTile = LayerTiles_T<CLICdetBarrelLayerTilesConstants>;
  using CLICdetBarrelTiles =
      std::array<CLICdetBarrelLayerTile, CLICdetBarrelLayerTilesConstants::nLayers>;

  using CLDEndcapLayerTile = LayerTiles_T<CLDEndcapLayerTilesConstants>;
  using CLDEndcapTiles =
      std::array<CLDEndcapLayerTile, CLDEndcapLayerTilesConstants::nLayers>;

  using CLDBarrelLayerTile = LayerTiles_T<CLDBarrelLayerTilesConstants>;
  using CLDBarrelTiles =
      std::array<CLDBarrelLayerTile, CLDBarrelLayerTilesConstants::nLayers>;

  using LArBarrelLayerTile = LayerTiles_T<LArBarrelLayerTilesConstants>;
  using LArBarrelTiles =
      std::array<LArBarrelLayerTile, LArBarrelLayerTilesConstants::nLayers>;
...
public:
  ...
  void fill(int index, float x, float y, float phi, unsigned int objectId) {
    tiles_[index].fill(x, y, phi, objectId);
  }
