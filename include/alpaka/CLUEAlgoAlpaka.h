...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  template <typename TAcc, typename TTile, typename TConvKernel = FlatKernel>
  ...
  public:
    ...
    CLUEAlgoAlpaka_T(
        float dc, float rhoc, float outlierDeltaFactor, Queue queue, bool verbose)
        : dc_{dc},
          rhoc_{rhoc},
      ...
      d_tiles = alpaka::allocAsyncBufIfSupported<TilesAlpaka_T<TAcc, TTile>, Idx>(
          queue, Vec1D{1});
      d_seeds =
          alpaka::allocAsyncBufIfSupported<alpakatools::VecArray<int32_t, max_seeds>, Idx>(
              queue, Vec1D{1});
      d_followers =
          alpaka::allocAsyncBufIfSupported<alpakatools::VecArray<int32_t, max_followers>,
                                           Idx>(queue, Vec1D{MAX_POINTS});
    ...
    std::optional<alpaka::Buf<Device, TilesAlpaka_T<TAcc, TTile>, Dim1D, Idx>> d_tiles;
    std::optional<alpaka::Buf<Device, VecArray<int32_t, max_seeds>, Dim1D, Idx>> d_seeds;
    std::optional<alpaka::Buf<Device, VecArray<int32_t, max_followers>, Dim1D, Idx>>
        d_followers;
    std::optional<PointsAlpaka<2>> d_points;

    void makeClusters(Points<2>& h_points, Queue& queue, std::size_t block_size);
    bool clearAndSetPoints(const Points<2>& h_points,
                           Queue& queue,
                           std::size_t block_size);

    std::map<int, std::vector<int>> getClusters(const Points<2>& h_points);
  ...
  bool CLUEAlgoAlpaka_T<TAcc, TTile, TConv>::clearAndSetPoints(const Points<2>& h_points,
                                                               Queue& queue,
                                                               std::size_t block_size) {
    if (not h_points.size())
      return false;

    d_points->nPoints = h_points.size();

    const Idx tiles_grid_size = alpakatools::divide_up_by(TTile::nTiles, block_size);
    ...
    alpaka::exec<Acc1D>(
        queue, work_div, KernelResetFollowers{}, d_followers->data(), h_points.size());
  ...
  void CLUEAlgoAlpaka_T<TAcc, TTile, TConvKernel>::makeClusters(Points<2>& h_points,
                                                                Queue& queue,
                                                                std::size_t block_size) {
    // #ifdef GPU_DEBUG
    ...
    std::cout << "ClueGaudiAlgorithmWrapper: KernelFillTiles:     " << elapsed << " us"
              << std::endl;
#endif
    ...
    elapsed = static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(finish - start).count());
    std::cout << "ClueGaudiAlgorithmWrapper: KernelCalculateLocalDensity: " << elapsed
              << " us" << std::endl;
#endif
    ...
    elapsed = static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(finish - start).count());
    std::cout << "ClueGaudiAlgorithmWrapper: KernelCalculateNearestHigher: " << elapsed
              << " us" << std::endl;
#endif
    ...
    elapsed = static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(finish - start).count());
    std::cout << "ClueGaudiAlgorithmWrapper: KernelFindClusters: " << elapsed << " us"
              << std::endl;
#endif
    ...
    auto work_div_seeds = alpakatools::make_workdiv<Acc1D>(grid_size_seeds, block_size);
#ifdef GPU_DEBUG
    start = std::chrono::high_resolution_clock::now();
#endif
    alpaka::exec<Acc1D>(queue,
    ...
    elapsed = static_cast<double>(
        std::chrono::duration_cast<std::chrono::microseconds>(finish - start).count());
    std::cout << "ClueGaudiAlgorithmWrapper: KernelAssignClusters: " << elapsed << " us"
              << std::endl;
#endif

    h_points.clusterIndex.resize(nPoints);
    h_points.isSeed.resize(nPoints);
    alpaka::memcpy(queue,
  ...
  template <typename TAcc, typename TTile, typename TConv>
  std::map<int, std::vector<int>> CLUEAlgoAlpaka_T<TAcc, TTile, TConv>::getClusters(
      const Points<2>& h_points) {
  ...
  template <typename TAcc, typename TTile, typename TConv>
  void CLUEAlgoAlpaka_T<TAcc, TTile, TConv>::clearLayerTiles(Queue& queue,
                                                             std::size_t block_size) {
